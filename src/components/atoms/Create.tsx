...
const QuestionInput: React.FC<{
  ...
  <input
    ...
    value={value}
    onChange={onChange}
    className="relative box-border w-full border-b-[1px] border-solid border-[#373737] bg-transparent text-[25px] outline-none text-center"
  />
);
...
const CorrectOptionInput: React.FC<{
...
}> = ({ questionIndex, options, value, onChange }) => {
  ...
  const handleOptionSelect = (selectedOption: string) => {
    onChange(questionIndex, selectedOption);
